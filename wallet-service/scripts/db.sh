...
COMMAND="${1:-}"
VALUE="${2:-}"
LIQUIBASE_URL="${LIQUIBASE_URL:-jdbc:postgresql://localhost:5433/wallet_service}"
LIQUIBASE_USERNAME="${LIQUIBASE_USERNAME:-postgres}"
LIQUIBASE_PASSWORD="${LIQUIBASE_PASSWORD:-postgres}"

mvn_liquibase() {
  mvn -pl wallet-service -DskipTests \
...
if [ -z "$COMMAND" ]; then
  echo "Usage: sh wallet-service/scripts/db.sh <status|update|rollback-last|tag|rollback-tag> [value]"
...
case "$COMMAND" in
  ...
  tag)
    if [ -z "$VALUE" ]; then
      echo "Usage: sh wallet-service/scripts/db.sh tag <tag>"
      exit 1
    fi
  ...
  rollback-tag)
    if [ -z "$VALUE" ]; then
      echo "Usage: sh wallet-service/scripts/db.sh rollback-tag <tag>"
      exit 1
    fi
