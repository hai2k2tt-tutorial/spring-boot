...
export function ProductDialog({ open, onClose, saving, submit }: FormDialogProps) {
  ...
  return (
    <Modal title="Create product" open={open} onClose={onClose}>
      <FormProvider {...form}>
        <form className="grid gap-4 sm:grid-cols-2" onSubmit={form.handleSubmit((values) => submit(() => createProduct({ ...values, shopId: toOptional(values.shopId), categoryId: toOptional(values.categoryId), imageUrl: toOptional(values.imageUrl) })))}>
