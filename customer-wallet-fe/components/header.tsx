...
function logout() {
  ...
  clearAccessToken();
  window.location.assign("/api/customer-wallet-fe/auth/keycloak-logout");
}
      ...
      <div className="mx-auto flex min-h-16 w-full max-w-6xl items-center justify-between gap-4 px-4 sm:px-6">
        ...
        <div className="flex min-w-0 items-center gap-2">
          ...
          {status === "loading" ? null : isAuthenticated ? (
            ...
            <Button type="button" variant="outline" size="sm" onClick={() => beginCrossAppLogin("customer")}><LogIn className="h-4 w-4" /> Login with customer SSO</Button>
          )}
        </div>
      </div>
    </header>
